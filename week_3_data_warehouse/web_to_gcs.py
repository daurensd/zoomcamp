...
def upload_to_gcs(bucket, object_name, local_file):
    ...
    blob.upload_from_path(from_path=path, to_path=path)
...
def web_to_gcs(year, service):
    for i in range(12):
        ...
        to_path = Path(f"data/{service}/{file_name}")

        # upload it to gcs 
        upload_to_gcs(BUCKET, from_path, to_path)
